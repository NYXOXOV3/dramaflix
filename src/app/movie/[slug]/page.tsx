      ...
      <div className="flex flex-col md:flex-row gap-8">
        ...
        <div className="flex-1 space-y-4">
          ...
          {!movie.isVipOnly && movie.freeEpisodes > 0 && (
            <p className="text-xs text-dark-500">{movie.freeEpisodes} free episodes · {movie.totalEpisodes} total episodes</p>
          )}
        </div>
      </div>
      ...
      <section className="mt-10">
        ...
        <div className="grid grid-cols-4 sm:grid-cols-6 md:grid-cols-8 lg:grid-cols-10 gap-2">
          {episodes.map((ep) => (
            <Link
              key={ep.id}
              href={ep.isFree ? `/watch/${movie.slug}/${ep.episodeNumber}` : `/watch/${movie.slug}/${ep.episodeNumber}`}
