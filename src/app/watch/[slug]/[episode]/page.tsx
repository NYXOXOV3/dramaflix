      ...
      <div className="flex flex-col lg:flex-row gap-6">
        ...
        <div className="flex-1">
            ...
            {isLocked ? (
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center bg-gradient-to-br from-dark-900 via-dark-950 to-dark-900 p-6">
                ...
                <p className="text-sm text-dark-400 text-center max-w-sm mb-6">
                  Episode {episodeNum} is locked for VIP members only. Upgrade your plan to unlock all episodes.
                </p>
          ...
          <div className="mt-4 space-y-2">
            ...
            <div className="flex items-center gap-3 text-sm text-dark-400">
              ...
              {isLocked ? (
                <>
                  <span>·</span>
                  <span className="flex items-center gap-1 text-accent font-medium"><Lock size={12} /> VIP Locked</span>
                </>
              ) : currentEp.isVipOnly ? (
                <>
                  <span>·</span>
          ...
          <div className="flex items-center gap-3 mt-4">
            ...
            {hasNext && (
              <Link
                href={episodes[episodeNum]?.isFree ? `/watch/${movie.slug}/${episodeNum + 1}` : `/watch/${movie.slug}/${episodeNum + 1}`}
        ...
        <div className="lg:w-72 xl:w-80 shrink-0">
            ...
            <div className="p-4 border-b border-dark-800 flex items-center justify-between">
              ...
              <div className="flex items-center gap-2 text-[10px]">
                <span className="text-success bg-success/10 px-1.5 py-0.5 rounded">
                  {movie.freeEpisodes} free
                </span>
                <span className="text-accent bg-accent/10 px-1.5 py-0.5 rounded flex items-center gap-0.5">
                  <Lock size={8} /> VIP
                </span>
              </div>
            </div>
            <div className="max-h-[500px] overflow-y-auto p-2 space-y-1">
              {episodes.map((ep) => {
                ...
                return (
                  <Link
                    key={ep.id}
                    href={`/watch/${movie.slug}/${ep.episodeNumber}`}
                  ...
                  >
                    <div className={`w-8 h-8 rounded-lg flex items-center justify-center text-xs font-bold shrink-0 ${
                      isCurrent ? "bg-accent text-dark-950" : isEpLocked ? "bg-accent/10 text-accent" : "bg-dark-800 text-dark-300"
                    }`}>
                    ...
                    {isEpLocked ? (
                      <span className="text-[10px] text-accent font-medium bg-accent/10 px-2 py-0.5 rounded shrink-0 flex items-center gap-0.5">
                        <Crown size={8} /> VIP
                      </span>
                    ) : (
          ...
          <div className="mt-4 p-4 bg-gradient-to-br from-accent/10 to-dark-900 border border-accent/20 rounded-2xl text-center">
            ...
            <Link
              href="/vip"
              className="inline-flex px-4 py-2 bg-accent hover:bg-accent-dark text-dark-950 font-bold text-xs rounded-lg transition-all"
            >
              View VIP Plans
            </Link>
          </div>
